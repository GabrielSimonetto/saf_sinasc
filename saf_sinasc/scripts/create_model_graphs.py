...
def plot_aggregated_feature_importances(data_version, evaluator, num_of_features_to_display=NUM_OF_FEATURES_TO_DISPLAY):
    ...
    list_of_series = [pd.Series(fi, c) for fi, c in zip(
        evaluator.feature_importances, evaluator.training_columns)]
    ...
    # Apparently... this was never a df?
    df_importances = (
        fi_mean.to_frame().reset_index().rename(
    ...
    save_path = MODEL_GRAPHS_OUTPUT_PATH / \
        data_version / f"{evaluator.name}_feature_importances.png"
...
def plot_from_result_metrics(data_version, evaluators, metric, agg_method):
    ...
    save_path = MODEL_GRAPHS_OUTPUT_PATH / \
        data_version / f"metric_graph_{metric}_{agg_method}.png"
...
def validate_data_version(value):
    if not value:
        raise argparse.ArgumentTypeError(
            "Data version cannot be an empty string.")
    return value
